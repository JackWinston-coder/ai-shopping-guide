...
async def cart_add(
    ...
    if cart_service is None:
        ...
        cart_service = CartService(_db_connection, product_service)
    if context is None:
        return {"success": False, "error": "缺少上下文"}
    try:
        item = await cart_service.add_item(context.user_id, product_id, sku_id, quantity)
        return {
            "success": True,
...
async def cart_remove(
    ...
    if context is None:
        return {"success": False, "error": "缺少上下文"}
    try:
        await cart_service.remove_item(context.user_id, item_id)
...
async def cart_update_quantity(
    ...
    if context is None:
        return {"success": False, "error": "缺少上下文"}
    try:
        item = await cart_service.update_quantity(context.user_id, item_id, quantity)
        return {
            "success": True,
...
async def cart_view(
    ...
    if context is None:
        return {"success": False, "error": "缺少上下文"}
    try:
        items = await cart_service.list_items(context.user_id)
        return {
            "success": True,
            "items": [
                {
